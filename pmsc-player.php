...
Plugin URI: https://github.com/pmusic/pmusic_wp_sc_player
...
class PMSCPlayer {
  ...
  function enqueue_scripts() {
	...
	function admin_page() {
		...
		do_settings_sections( 'pmsc_player_options' );
		submit_button();
		echo '</form>';
	}

	function admin_page_info() {
		echo '<div>Enter the soundcloud API info here. If you do not have a SoundCloud Client ID or Client Secret, you can get one by logging in to SoundCloud and registering your application <a href="http://soundcloud.com/you/apps">here</a>.</div>';
	}
  ...
  function admin_init() {
    ...
    add_settings_section( 'scapi', 'SoundCloud API Keys', array($this, 'admin_page_info'), 'pmsc_player_options' );
    add_settings_field( 'client_id', 'Client ID', array($this, 'print_text_input'), 'pmsc_player_options', 'scapi', array('field'=>'client_id') );
    add_settings_field( 'client_secret', 'Client Secret', array($this, 'print_text_input'), 'pmsc_player_options', 'scapi', array('field'=>'client_secret') );
  }
  ...
  function big_player( $json_pl ) {
    ...
    $r = <<<EOF
<div id="pmsc-p$pl->id" class="pmsc-player pmsc-500" style="background-image:url('$img_url')">
	<div class="footer">		'
